      ...
      <aside className="hidden lg:flex flex-col w-56 shrink-0 border-r border-line bg-surface-1/60 sticky top-0 h-screen">
        ...
        <div className="px-4 py-3 border-t border-line text-2xs text-ink-low leading-relaxed">
          <div className="flex items-center gap-1.5 mb-1">
            <Sparkles size={11} className={app.realConnected ? 'text-ok-400' : 'text-brand-300'} />
            <span className="font-semibold text-ink-mid">{app.realConnected ? 'Live data connected' : 'Simulation workspace'}</span>
          </div>
          {app.realConnected
            ? 'AppsFlyer is the source of truth for cost, attributed installs, revenue and retention across Meta, TikTok and organic. Product events are not connected. Decisions, config and audit are local.'
